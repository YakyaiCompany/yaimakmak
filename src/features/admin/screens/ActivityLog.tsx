  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_190px_190px]">
        ...
        <label><span className="sr-only">กรองการเปลี่ยนแปลง</span><select value={actionFilter} onChange={(event) => setActionFilter(event.target.value as ActivityAction | "ทั้งหมด")} className={`w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-medium shadow-sm ${focusRing}`}><option>ทั้งหมด</option><option>เพิ่ม</option><option>แก้ไข</option><option>เผยแพร่</option><option>ยกเลิกเผยแพร่</option><option>ลบ</option></select></label>
